 ...
 */
export function runCRTAnalysis(htf, ltf, sym) {
  ...
  if (ltf && ltf.length >= 3) {
    ...
    // If no FVG found, synthesise one near current price
    if (!fvg) {
      const lp  = ltf[ltf.length - 1].close;
      const gap = lp * 0.0012;
      fvg =
        sweepDir === "bullish"
          ? { type: "bullish", top: lp - gap * 0.4, bottom: lp - gap * 1.4, mid: lp - gap * 0.9 }
          : { type: "bearish", top: lp + gap * 1.4, bottom: lp + gap * 0.4, mid: lp + gap * 0.9 };
    }
  }
