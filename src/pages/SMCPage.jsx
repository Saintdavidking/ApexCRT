...
// ── SMC trade plan is generated via the same CRT engine
//    but framed in SMC language (OB retests, BOS targets)
function buildSMCPlan(analysis) {
  if (!analysis) return null;
  return {
    ...analysis,
    smcEntry:  analysis.entry,
    smcSL:     analysis.sl,
    smcTP:     analysis.tp,
    entryType: analysis.bias === "LONG"  ? "Bullish OB / Discount FVG" : "Bearish OB / Premium FVG",
    target:    analysis.bias === "LONG"  ? "BOS above range high"        : "BOS below range low",
    model:     analysis.bias === "LONG"  ? "PD Array Long"               : "PD Array Short",
  };
}
...
// ── SMC trade plan panel (wraps TradePlanPanel with SMC labels) ───
function SMCTradePlan({ loading, analysis, dataSource }) {
  const plan = buildSMCPlan(analysis);
  ...
  const rrVal     = parseFloat(plan.rr);
  ...
  return (
    <div style={{ padding: 14, display: "flex", flexDirection: "column", gap: 9, overflowY: "auto", height: "100%" }}>
      ...
      <div style={{ background: `${biasColor}15`, border: `1px solid ${biasColor}40`, borderRadius: 9, padding: "11px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ color: G.sub, fontSize: 11, letterSpacing: "0.06em" }}>SMC BIAS</span>
      ...
      <div style={{ background: G.surface, borderRadius: 9, padding: "10px 14px" }}>
        <div style={{ color: G.dim, fontSize: 10, letterSpacing: "0.06em", marginBottom: 4 }}>PD ARRAY MODEL</div>
        <div className="syne" style={{ fontWeight: 600, fontSize: 13, color: G.blue }}>{plan.model}</div>
      </div>

      {/* Entry type */}
      <div style={{ background: G.surface, borderRadius: 9, padding: "10px 14px", border: `1px solid ${G.purple}28` }}>
        <div style={{ color: G.dim, fontSize: 10, letterSpacing: "0.06em", marginBottom: 4 }}>ENTRY TYPE</div>
        <div className="syne" style={{ fontWeight: 600, fontSize: 13, color: G.purple }}>{plan.entryType}</div>
      </div>

      {/* HTF range */}
      <div style={{ background: G.surface, borderRadius: 9, padding: "10px 14px" }}>
        <div style={{ color: G.dim, fontSize: 10, letterSpacing: "0.07em", marginBottom: 7 }}>STRUCTURAL RANGE</div>
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          {[["High", plan.rangeHigh, G.red], ["Mid", plan.rangeMid, G.sub], ["Low", plan.rangeLow, G.green]].map(([l, v, c]) => (
            <div key={l}>
      ...
      {/* Liquidity sweep */}
      <div style={{ background: G.surface, borderRadius: 9, padding: "10px 14px" }}>
        <div style={{ color: G.dim, fontSize: 10, letterSpacing: "0.07em", marginBottom: 5 }}>LIQUIDITY SWEPT</div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span className="syne" style={{ color: biasColor, fontWeight: 600, fontSize: 13 }}>
            {isLong ? "↓ SSL Swept" : "↑ BSL Swept"}
          </span>
          <span style={{ color: G.sub, fontSize: 12 }}>{plan.sweepLevel}</span>
        </div>
      </div>
      ...
      {plan.fvg && (
        <div style={{ background: G.surface, borderRadius: 9, padding: "10px 14px", border: `1px solid ${G.blue}28` }}>
          <div style={{ color: G.dim, fontSize: 10, letterSpacing: "0.07em", marginBottom: 5 }}>FVG / IMBALANCE</div>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            {[["Top", plan.fvg.top], ["Mid", plan.fvg.mid], ["Bot", plan.fvg.bottom]].map(([l, v]) => (
              <div key={l}>
                <div style={{ color: G.dim, fontSize: 10 }}>{l}</div>
      ...
      {/* Target */}
      <div style={{ background: G.surface, borderRadius: 9, padding: "10px 14px" }}>
        <div style={{ color: G.dim, fontSize: 10, letterSpacing: "0.06em", marginBottom: 4 }}>STRUCTURAL TARGET</div>
        <div className="syne" style={{ fontWeight: 600, fontSize: 12, color: G.accent }}>{plan.target}</div>
      </div>

      {/* Entry */}
      <div style={{ background: `${G.accent}12`, border: `1px solid ${G.accent}30`, borderRadius: 9, padding: "10px 14px" }}>
        <div style={{ color: G.dim, fontSize: 10, letterSpacing: "0.07em", marginBottom: 4 }}>ENTRY PRICE</div>
        <div className="syne" style={{ fontWeight: 700, fontSize: 20, color: G.accent }}>{plan.entry}</div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        <div style={{ background: `${G.red}10`, border: `1px solid ${G.red}30`, borderRadius: 9, padding: "10px 12px" }}>
          ...
          <div className="syne" style={{ fontWeight: 700, fontSize: 15, color: G.red }}>{plan.sl}</div>
        </div>
        <div style={{ background: `${G.green}10`, border: `1px solid ${G.green}30`, borderRadius: 9, padding: "10px 12px" }}>
          ...
          <div className="syne" style={{ fontWeight: 700, fontSize: 15, color: G.green }}>{plan.tp}</div>
        </div>
      </div>
      ...
      <div style={{ background: G.surface, borderRadius: 9, padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <span className="syne" style={{ fontWeight: 800, fontSize: 20, color: rrColor }}>1 : {plan.rr}</span>
      </div>
  ...
  const runAnalysis = useCallback(async () => {
    ...
    try {
      ...
      const parse = (d) => {
        ...
        return q.open.map((o, i) => ({ open: o||0, high: q.high[i]||0, low: q.low[i]||0, close: q.close[i]||0 })).filter(c => c.high && c.low && c.close);
      };
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 22, flexWrap: "wrap", gap: 14 }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 5, flexWrap: "wrap" }}>
            ...
            <Pill color={G.blue} filled>LIVE</Pill>
            {sym.type && (
        ...
        <div style={{ display: "flex", gap: 10, alignItems: isMobile ? "stretch" : "flex-end", flexWrap: "wrap", width: isMobile ? "100%" : undefined, flexDirection: isMobile ? "column" : "row" }}>
          ...
          <button className="btn" onClick={runAnalysis} disabled={loading} style={{ background: G.blue, color: "#fff", padding: "8px 18px", borderRadius: 7, fontFamily: "Syne", fontWeight: 700, fontSize: 13, opacity: loading ? 0.65 : 1, width: isMobile ? "100%" : undefined }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr 310px", gap: 14, marginBottom: 14 }}>
        ...
        <div style={{ background: G.card, border: `1px solid ${G.border}`, borderRadius: 13, overflow: "hidden" }}>
          <div style={{ padding: "9px 14px", borderBottom: `1px solid ${G.border}`, display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            ...
            <span className="syne" style={{ fontWeight: 600, fontSize: 13, color: G.blue }}>HTF — {htfTf.label}</span>
        ...
        <div style={{ background: G.card, border: `1px solid ${G.border}`, borderRadius: 13, overflow: "hidden" }}>
          <div style={{ padding: "9px 14px", borderBottom: `1px solid ${G.border}`, display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            ...
            <span className="syne" style={{ fontWeight: 600, fontSize: 13, color: G.purple }}>LTF — {ltfTf.label}</span>
        ...
        {SMC_REFS.map(r => (
          <div key={r.label} style={{ background: G.card, border: `1px solid ${G.border}`, borderRadius: 10, padding: 15 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 7 }}>
              ...
              <span className="syne" style={{ fontWeight: 600, fontSize: 13, color: r.color }}>{r.label}</span>
            </div>
